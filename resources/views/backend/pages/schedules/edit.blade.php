                ...
                <form action="{{ route('admin.schedules.update', $schedule->id) }}" method="POST">
                            ...
                            <select name="order_id" class="form-control" placeholder="Order">
                                ...
                                @foreach ($orders as $i => $order)
                                    <option value="{{ $order->id }}" 
                                        ...
                                        / {{ $order->gigPackage?->title }} / {{ $order->customer->nickname }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="form-group">
                            ...
                            <select name="gig_id" class="form-control" placeholder="Package">
                                ...
                                @foreach ($gigs as $i => $gig)
                                    <option value="{{ $gig->id }}"
                                        @selected($gig->id == $schedule->gig_id)>{{ $gig->title }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                            ...
                            <select name="artist_id" class="form-control" placeholder="Artist">
                                ...
                                @foreach ($artists as $i => $artist)
                                    <option value="{{ $artist->id }}"
                                        @selected($artist->id == $schedule->artist_id)>{{ $artist->name }} / {{ $artist->nickname }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select name="customer_id" class="form-control" placeholder="Customer">
                                ...
                                @foreach ($customers as $i => $customer)
                                    <option value="{{ $customer->id }}" @selected($customer->id == $schedule->customer_id)>{{ $customer->name }} / {{ $customer->nickname }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="col-xs-12 col-sm-12 col-md-12">
                        <div class="form-group">
                            ...
                            <input type="date" name="start_date" value="{{ $schedule->start_date }}" class="form-control" placeholder="Start Date">
                        </div>
                    </div>
                    ...
                    <div class="col-xs-12 col-sm-12 col-md-12">
                        <div class="form-group">
                            ...
                            <input type="date" name="end_date" value="{{ $schedule->end_date }}" class="form-control" placeholder="End Date">
                        </div>
                    </div>
                            ...
                            <select name="status" class="form-control" placeholder="Status">
                                ...
                                @foreach ($schedule_status as $i => $status)
                                    <option value="{{ $status->id }}"
                                        @selected($status->id == $schedule->status)
